# Control Unit Module — Technical Documentation

## Overview

The `control_unit.sv` module decodes a RISC-V instruction and drives the full execute-stage control bundle. It is combinational: from `instr_type` and opcode/funct fields it selects ALU op, memory access shape, register write, CSR access, and branch/jump class.

## Main responsibilities

1. **ALU control:** choose operation (ADD, SUB, XOR, …)  
...
| `instr_type_i` | instr_type_e | Instruction class from fetch pre-decode |

### Output ports

| Port | Type | Description |
|------|-----|----------|
| `ctrl_o` | ctrl_t | Full execute-stage control bundle |
...
**RISC-V RV32I rules:**
- Shift amount is 5 bits (0–31).  
- On RV64 the amount can be 6 bits; on RV32, `imm[5]` = `inst_i[25]` must be zero.  
- Otherwise raise `ILLEGAL_INSTRUCTION`.  
...
// Is the CSR implemented?
csr_supported = is_supported_csr(inst_i[31:20]);

// Read-only CSR? (bits [11:10] == 2'b11)
...
- `csr[11:10] == 2'b11` → Read-only CSR (e.g. mvendorid, marchid, cycle)
- `csr[11:10] == 2'b00` → Read-write CSR (e.g. mstatus, mtvec)
...
**Note:** Loads, stores, and branches still use `OP_ADD` for effective-address formation.  
...
The control unit is largely one big case over opcode / `instr_type` that assigns `ctrl_o`:  
...
**Example:** `add x1, x2, x3` → ALU: x2 + x3 → x1
...
**Example:** `addi x1, x2, 12` → ALU: x2 + 12 → x1
...
**Example:** `lw x1, 8(x2)` → Memory[x2 + 8] → x1
...
**Example:** `sw x3, 8(x2)` → x3 → Memory[x2 + 8]
...
**Example:** `beq x1, x2, offset` → if (x1 == x2) PC = PC + offset
...
**Example:** `jal x1, offset` → x1 = PC+4, PC = PC + offset
...
**Example:** `jalr x1, 0(x2)` → x1 = PC+4, PC = x2 + 0
...
**Example:** `auipc x1, 0x12345` → x1 = PC + (0x12345 << 12)
...
**Note:** These opcodes need dedicated handling in execute (trap, privilege, etc.).  
...
| 2'b01 | imm | JALR (PC + offset uses immediate path) |
...
**Total combinational delay:** ~7–10 logic levels (one cycle in typical timing)

### Area optimization

- **Shared cases:** many instruction types share the same `ctrl_o` assignments  
- **Defaults:** pre-assign defaults then override in specific branches (helps synthesis)  
- **CSR helper:** `is_supported_csr()` centralizes the implemented-CSR set  
...
```systemverilog
// R-type: register write must be enabled
assert property (@(posedge clk_i)
  (inst_i.opcode == op_r_type) |-> ctrl_o.rf_rw_en);

// S-type: memory write must be enabled
assert property (@(posedge clk_i)
  (inst_i.opcode == op_s_type) |-> ctrl_o.wr_en);

// Load: data cache access must be valid
assert property (@(posedge clk_i)
  (inst_i.opcode == op_i_type_load) |-> ctrl_o.dcache_valid);

// Branch: must not write the integer regfile
assert property (@(posedge clk_i)
...
1. **decode.sv:** instantiates the control unit  
2. **execute.sv:** consumes `ctrl_o`  
3. **csr_file.sv:** CSR read/write side of `ctrl_o`  
4. **alu.sv:** opcode from `ctrl_o.alu_ctrl`  
