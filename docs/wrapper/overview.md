# Level SoC - Full System-on-Chip Wrapper Technical Documentation
...
5. [CLINT Implementation](#clint-implementation)
6. [Peripheral Integration](#peripheral-integration)
7. [RAM and Programming](#ram-and-programming)
...
The `cpu` module provides a **full-featured SoC** implementation. It integrates the CPU, RAM, and all peripherals into a runnable system.
...
#(
    ...
    parameter string PROGRAM_SEQUENCE_P = PROGRAM_SEQUENCE  // "LEVELTEST"
)
```
...
1. **CPU Integration**: Level RV32IMC core
...
This module provides a full-featured SoC platform.
...
1. [Overview](#overview)
2. [Module Interface](#module-interface)
3. [Programming Protocol](#programming-protocol)
4. [FSM Details](#fsm-details)
5. [UART Receiver](#uart-receiver)
6. [RAM Writes](#ram-yazma)
7. [Reset Management](#reset-management)
...
│   │         │    │              │    │            │    │             │         │
│   └─────────┘    │ "LEVELTEST"  │    │ Word Count │    │ Data Words  │         │
│       ▲          └──────────────┘    └────────────┘    └──────┬──────┘         │
│       │                                                       │                 │
│       │          ┌──────────────────────────────────────────┐ │                 │
│       └──────────│              FINISH                      │◄┘                 │
...
│   │ "LEVELTEST" │  │ N             │  │ word[0], word[1], ... word[N-1] │  │
│   └─────────────┘  └───────────────┘  └─────────────────────────────────┘  │
│                                                                             │
│   Total bytes = 9 + 4 + (N × 4)                                            │
│                                                                             │
...
│                                                                             │
│   "LEVELTEST" + 0x00000100 (LE) + 256 × 32-bit words                       │
...
```
              ...
              ┌───────────┐      ┌─────┐    ┌───────────────────┐
i_uart_rx ────┤LEVELTEST  ├──────┤ N   ├────┤ W0  W1  W2 ... Wn ├────────
              └───────────┘      └─────┘    └───────────────────┘
...
The `wb_clint_slave` module exposes **RISC-V Core Local Interruptor (CLINT)** functionality over a Wishbone slave interface. It provides timer and software interrupt handling.
...
The `wb_pbus_slave` module bridges from the Wishbone bus to peripheral modules. It translates address and control signals to a simple memory-mapped interface.
...
The `wb_ram_slave` module interfaces the Wishbone bus to a RAM module. It supports configurable latency and byte-granular writes.
...
The `wrapper_ram` module is a RAM with **128-bit cache-line** reads and **byte-granular writes**. It also includes a UART programming interface.
