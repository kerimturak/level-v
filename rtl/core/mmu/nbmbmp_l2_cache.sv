...
(
  ...
  assign i_sram_idx = flush_active ? flush_cnt : (i_pipe_state == PIPE_IDLE) ? i_next_set : i_req_set;

  assign d_sram_idx = (d_pipe_state == PIPE_IDLE) ? d_next_set : d_req_set;
  ...
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      ...
      if (i_dirty_wr && d_dirty_wr && i_dirty_idx == d_dirty_idx) begin
        ...
        if (i_dirty_wr) for (int w = 0; w < L2_NUM_WAY; w++) if (i_dirty_way[w]) dirty_reg[i_dirty_idx][w] <= i_dirty_val;
        if (d_dirty_wr) for (int w = 0; w < L2_NUM_WAY; w++) if (d_dirty_way[w]) dirty_reg[d_dirty_idx][w] <= d_dirty_val;
      end
    end
  end
  ...
  always_comb begin
    ...
    i_select_data = '0;
    for (int w = 0; w < L2_NUM_WAY; w++) if (i_hit_way_oh[w]) i_select_data = i_data_rdata[w];
  end
  ...
  always_comb begin
    ...
    d_hit_way_bin = '0;
    for (int w = L2_NUM_WAY - 1; w >= 0; w--) if (d_hit_way_oh[w]) d_hit_way_bin = L2_WAY_W'(w);
    d_select_data = '0;
    for (int w = 0; w < L2_NUM_WAY; w++) if (d_hit_way_oh[w]) d_select_data = d_data_rdata[w];
  end
  ...
  always_comb begin
    ...
    for (int b = 0; b < BLK_SIZE / 8; b++) if (d_req_q.wstrb[b]) d_hit_wr_merged[b*8+:8] = d_req_q.wdata[b*8+:8];
  end
  ...
  always_comb begin
    for (int i = 0; i < L2_MSHR_DEPTH; i++) begin
      i_mshr_line_match[i] = mshr_entries[i].valid && (mshr_entries[i].addr[L2_ADDR_WIDTH-1:L2_OFFSET_BITS] == i_req_q.addr[L2_ADDR_WIDTH-1:L2_OFFSET_BITS]);
      d_mshr_line_match[i] = mshr_entries[i].valid && (mshr_entries[i].addr[L2_ADDR_WIDTH-1:L2_OFFSET_BITS] == d_req_q.addr[L2_ADDR_WIDTH-1:L2_OFFSET_BITS]);
    end
  ...
  always_comb begin
    for (int i = 0; i < L2_MSHR_DEPTH; i++) mshr_free_vec[i] = !mshr_entries[i].valid;
    mshr_any_free = |mshr_free_vec;
    mshr_free_idx = '0;
    for (int i = L2_MSHR_DEPTH - 1; i >= 0; i--) if (mshr_free_vec[i]) mshr_free_idx = L2_MSHR_PTR_W'(i);
  end
  ...
  always_comb begin
    for (int i = 0; i < L2_MSHR_DEPTH; i++) mshr_pending_vec[i] = mshr_entries[i].valid && (mshr_entries[i].state == MSHR_PENDING);
    mshr_pending_valid = |mshr_pending_vec;
    mshr_pending_idx   = '0;
    for (int i = L2_MSHR_DEPTH - 1; i >= 0; i--) if (mshr_pending_vec[i]) mshr_pending_idx = L2_MSHR_PTR_W'(i);
  ...
  always_comb begin
    for (int i = 0; i < L2_MSHR_DEPTH; i++) mshr_fill_match_vec[i] = mshr_entries[i].valid && (mshr_entries[i].state == MSHR_FILL_ACTIVE);
    mshr_fill_entry_idx = '0;
    for (int i = L2_MSHR_DEPTH - 1; i >= 0; i--) if (mshr_fill_match_vec[i]) mshr_fill_entry_idx = L2_MSHR_PTR_W'(i);
    mshr_fill_from_dport = mshr_entries[mshr_fill_entry_idx].from_dport;
  end

  always_comb begin
    for (int i = 0; i < L2_MSHR_DEPTH; i++) mshr_complete_vec[i] = mshr_entries[i].valid && (mshr_entries[i].state == MSHR_COMPLETE);
  ...
  assign d_mshr_do_alloc = d_mshr_alloc_req && !d_mshr_any_match && !d_mshr_full && !(i_mshr_do_alloc && mshr_free_vec == (1 << mshr_free_idx));
  ...
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      for (int i = 0; i < L2_MSHR_DEPTH; i++) mshr_entries[i] <= '0;
    end else begin
      // I-pipe allocation
      ...
      if (d_mshr_do_alloc) begin
        ...
        if (i_mshr_do_alloc) begin
          d_idx = '0;
          for (int i = L2_MSHR_DEPTH - 1; i >= 0; i--) if (mshr_free_vec[i] && L2_MSHR_PTR_W'(i) != mshr_free_idx) d_idx = L2_MSHR_PTR_W'(i);
        end else begin
          d_idx = mshr_free_idx;
        end
  ...
  always_comb begin
    ...
    end else if (d_bypass_active && !cached_req.valid) begin
      mem_req_o.valid    = 1'b1;
      mem_req_o.ready    = 1'b1;
  ...
  // =========================================================================
  // Bypass in-flight tracking
  // =========================================================================
  // Track whether each pipe's bypass request has actually been sent to the
  // WB bridge.  Without this, a fill-completion valid pulse could be
  // mistaken for a bypass response and return wrong data.
  wire i_bypass_driving = i_bypass_active && !cached_req.valid;
  wire d_bypass_driving = d_bypass_active && !cached_req.valid && !i_bypass_active;

  wire i_bypass_accepted = i_bypass_driving && mem_res_i.ready;
  wire d_bypass_accepted = d_bypass_driving && mem_res_i.ready;

  logic i_bypass_inflight_q, d_bypass_inflight_q;

  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      i_bypass_inflight_q <= 1'b0;
      d_bypass_inflight_q <= 1'b0;
    end else begin
      if (i_pipe_state != PIPE_BYPASS) i_bypass_inflight_q <= 1'b0;
      else if (i_bypass_accepted) i_bypass_inflight_q <= 1'b1;

      if (d_pipe_state != PIPE_BYPASS) d_bypass_inflight_q <= 1'b0;
      else if (d_bypass_accepted) d_bypass_inflight_q <= 1'b1;
    end
  end

  wire i_bypass_done = (i_bypass_inflight_q || i_bypass_accepted) && mem_res_i.valid;
  wire d_bypass_done = (d_bypass_inflight_q || d_bypass_accepted) && mem_res_i.valid;
  ...
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      ...
      unique case (i_pipe_state)
        ...
        PIPE_BYPASS: if (i_bypass_done) i_pipe_state <= PIPE_IDLE;
  ...
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      ...
      unique case (d_pipe_state)
          ...
          if (d_pipe_accept) begin
            ...
            if (dcache_req_i.rw) begin
              unique case (dcache_req_i.rw_size)
                ...
                default: d_req_q.wstrb <= dcache_req_i.uncached ? ((BLK_SIZE / 8)'(4'b1111) << dcache_req_i.addr[BOFFSET-1:0]) : '1;
              endcase
            end else begin
              d_req_q.wstrb <= '0;
            end
        ...
        PIPE_BYPASS: begin
          if (d_bypass_done) d_pipe_state <= PIPE_IDLE;
        end
  ...
  always_comb begin
    ...
    end else if (i_bypass_done) begin
      i_resp_valid = 1'b1;
  ...
  // Helper functions
  // =========================================================================
  function automatic logic [BLK_SIZE-1:0] merge_fill_data(input logic [BLK_SIZE-1:0] fill, input logic [BLK_SIZE-1:0] wdata, input logic [BLK_SIZE/8-1:0] wstrb);
    logic [BLK_SIZE-1:0] merged;
    merged = fill;
    for (int b = 0; b < BLK_SIZE / 8; b++) if (wstrb[b]) merged[b*8+:8] = wdata[b*8+:8];
    return merged;
  endfunction

  function automatic [L2_NUM_WAY-2:0] update_node(input logic [L2_NUM_WAY-2:0] node_in, input logic [L2_NUM_WAY-1:0] hit_vec);
  ...
  function automatic [L2_NUM_WAY-1:0] compute_evict_way(input logic [L2_NUM_WAY-2:0] node_in);
