...
// On 32-bit platforms, we need to concatenate the 32-bit `mcycle` and `mcycleh` CSRs. However, we need to be careful
    ...
    asm volatile (
        "%=:\n\t"
        "la t0, %2\n\t"      // Load the address into register t0
        "lw %1, 4(t0)\n\t"   // Load the higher 32 bits of the timer
        "lw %0, 0(t0)\n\t"   // Load the lower 32 bits of the timer
        "lw t1, 4(t0)\n\t"   // Load the higher bits again
        "bne %1, t1, %=b"
    : "=r" (cycle_lo), "=r" (cycle_hi)
    : "i" (0x30000000)       // Immediate value as input
    : "t0", "t1"
    );
