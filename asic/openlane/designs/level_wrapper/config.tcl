# ==============================================================
# Level RISC-V OpenLane Configuration (SKY130)
# ==============================================================

set ::env(DESIGN_NAME) level_wrapper

set sv2v_file "$::env(DESIGN_DIR)/src/level_wrapper_sv2v.v"
if {[file exists $sv2v_file]} {
	set ::env(VERILOG_FILES) $sv2v_file
} else {
	set pkg_file "$::env(DESIGN_DIR)/src/level_param.sv"
...
set ::env(SYNTH_DEFINES) "SYNTHESIS MINIMAL_SOC LEVEL_OPENLANE"
...
# Manual macro placement (place SRAMs by hand)
...
# sv2v parametric-type mangling emits complex expressions that cause Yosys
# false-positive "out of bounds" warnings when building RTLIL with default
# parameters. Actual elaboration runs with the correct parameters.
